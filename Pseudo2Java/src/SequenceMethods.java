...
public class SequenceMethods 
{
	...
	public void input ( String szLine ) 
	{
		...
		{
			// we need to get rid of the output and 'INPUT'
			// first get the equals index ( the index of the first '=' )
			for ( i = 0 ; i < szLine.length() ; i ++ )
			{
				// if the equals index is -1 and the current char ( via i )
				if ( iEqualsIndex == -1 && szLine.charAt(i) == '=' )
				{
					// then set the equals index to i
					iEqualsIndex = i ; 
				} // end if 
			} // end for 

			// then replace the command with the actual value, by using .substring
			szLine = szLine.substring( 0 , iEqualsIndex + 1 ) + szVariableValue ; 

			// get the necessary information, by removing the INPUT section
			vm.getVariableData ( szLine ) ; 
		} // end else ( getting the input )
